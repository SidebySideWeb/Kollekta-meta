...
async function stepContainer(client, ctx) {
  ...
  const root = instanceRoot(client.subdomain);
  const envFile = prodEnvPath(client.subdomain);

  if (!fs.existsSync(envFile)) {
    throw new Error(`Missing prod.env at ${envFile}`);
  }

  // Retry of this step: remove a broken/partial container with the same name.
  try {
  ...
  const binding = await assertLoopbackBinding(name, ctx.port);
  return `Started ${name} (${id || 'ok'}) bound ${binding}`;
}

async function stepHealthcheck(client, ctx) {
  const urls = [
    `http://127.0.0.1:${ctx.port}/`,
    `http://127.0.0.1:${ctx.port}/api/branding`,
  ];
